 ...
 */
public class IADifficileAntiquite extends IADifficile {
	...
	public String choixAction(InfoPlateau p, ArrayList<Outil> outilsj, ArrayList<Ouvrier> ouvriersj,
		...
		if (nbActions == 0) {
			...
			int poidsaffranchiresclave = poidsAffranchirEsclave(ouvriersj, ptsvictoire);
			...
			if (poidsaffranchiresclave == 2)
				return "AffranchirEsclave";
		...
		return actions.get(actionsfinal.get(new Random().nextInt(actionsfinal.size())));
	}
